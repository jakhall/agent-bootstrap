...
Read `.agents/project/discovery.md` before substantive work when it exists, then use the smallest matching set of skills for the task. Planning and observability are mandatory baselines from `.agents/AGENTS.md`; the skills below provide the deeper workflow when those concerns need explicit handling.
...
- `task-tracking`: trigger when a task needs persistent notes, explicit ownership, or a clean handoff across turns or agents. Create `.agents/project/tasks/` on demand and store task state there.
...
- Feature work: `code-planning` + `test-strategy`, plus `observability-review` when the flow is stateful or cross-boundary.
- Bug fix: `bug-investigation` + `test-strategy`, plus `observability-review` when current signals are weak.
- Code review: `code-review`, plus `observability-review` when diagnosability or operability matters.
- Unfamiliar repo: `project-discovery` first, then the task-specific set.
- Contract or migration change: `code-planning` + `contracts-and-migrations` + `test-strategy`, plus `observability-review` when rollout or compatibility visibility matters.
